...
const LS_NAME = '01_guy_dagashi_games_byhands';
...
function attachEnevntListners(){
  ...
  $medalSet.on("click", ()=> setMedalLS(gameStatus.medal));
  $medalGet.on("click", getMedalFromLS);
...
function setMedalLS(medal){
  // if(gameStatus.isGaming) return;
  console.log('setMedalLS(medal)'+medal);
  setLS(medal);
  // gameStatus.medal = 0;
  updateMedalInfo();
}

function getMedalFromLS(){
  // if(gameStatus.isGaming) return;
  // gameStatus.medal += getLS()||0;
  // deleteGookie();
  const LSResult = getLS();
  if(!LSResult) return 0;
...
function medalPayment(num) {
  gameStatus.medal = num + getLS();
  console.log('gameStatus.medal' + gameStatus.medal);
  setMedalLS(gameStatus.medal);
  updateMedalInfo();
}

function medalConsumption() {
  gameStatus.medal = getLS() - 1;
  setMedalLS(gameStatus.medal);
  updateMedalInfo();
}

function updateMedalInfo() {
const medalnow = getMedalFromLS();
...
function setLS(medal){
  // $.cookie(LS_NAME,medal);
  localStorage(LS_NAME,medal);
  console.log('LS_NAME:'+ LS_NAME + ' medal:' + medal);
}
function getLS(){
  // return Number($.cookie(LS_NAME));
  return Number(localStorage(LS_NAME));

}
function deleteGookie(){
  // $.removeCookie(LS_NAME);
